...
WHITE = "#dcdcdc" # black/white
TAN = "#b3b3b3" # black/tan
var = {'front':[], 'back':[], 'decimal':False, 'x_val':0.0, 'y_val':0.0, 'result':0.0, 'operator':''}

root = Tk()
root.title('Calculator')
root.config(bg='#d17379')

display_text = StringVar()
display_text.set('0')

calf = Canvas(bd=0, highlightthickness=0)
calf.pack(padx=15, pady=15)

def std_btn(text, bg, row, col, width=7, height=2, font=('arial',20,'bold')):
    btn = Button(calf, text=text, bg=bg, width=width, height=height, font=font, command=lambda: event_click(text))
    return btn.grid(row=row, column=col, padx=4, pady=4)

txtDisplay = Label(calf, text='PyDataMath-II', anchor='e', bg='#272533', fg='white', font=('arial',20,'bold'))
txtDisplay.grid(row=0, columnspan=4, sticky='ew', padx=4, pady=2)

Label(calf, textvariable=display_text, anchor='e', bg='black', fg='white', font=('Digital-7',47)).grid(row=1, columnspan=4, sticky='ew', padx=4, pady=2)
...
rtn_btn = Button(calf, text='=', bg='#ECA527', width=15, height=2, font=('Franklin Gothic Book', 24), command=lambda: event_click("="))
rtn_btn.focus()
